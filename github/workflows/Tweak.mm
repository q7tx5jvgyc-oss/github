...
// الدالة الأساسية التي تعمل تلقائياً فور تشغيل اللعبة
__attribute__((constructor)) static void initialize_key_system() {
    // تأخير العرض لثانية واحدة لضمان تحميل واجهة اللعبة الأصلية أولاً
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [alert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            ...
            textField.secureTextEntry = YES; // إخفاء الحروف بنقاط لحماية المفتاح أثناء الكتابة
        }];
        
        // 3. إضافة زر الدخول والتحقق من المفتاح الخاص بك
        UIAlertAction *confirm = [UIAlertAction actionWithTitle:@"دخول" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            // التحقق من تطابق المفتاح المكتوب مع مفتاحك الخاص
            if ([keyField.text isEqualToString:@"MOSTAHAU9801"]) {
                // المفتاح صحيح: تختفي الواجهة وتفتح اللعبة طبيعي
                NSLog(@"Access Granted!");
            } else {
                // المفتاح خاطئ: يتم إغلاق اللعبة فوراً لحماية حقوقك من السرقة
                exit(0); 
            }
        }];
